...
export class GeminiLiveRuntime {
  ...
  async start({
    ...
    try {
      ...
      const websocketUrl = validateGeminiRelayUrl(sessionPayload.websocketUrl);
...
async function defaultFetchToken() {
  const response = await fetch(apiUrl("/api/gemini-live/session"), {
    method: "POST",
    headers: {
...
function createGeminiRelayTransport({ url, onOpen, onMessage, onError, onClose }) {
  const relayUrl = validateGeminiRelayUrl(url);
